  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (titleFound && authorFound) {
      ...
      if (isHeader) {
        ...
        // Проверяем, содержит ли строка аккорды и выделяем их
        formattedText += highlightChords(line) + '\n';
      } else {
        // Пустая строка
        formattedText += '\n';
      }
...
// Функция для выделения аккордов в тексте
function highlightChords(text) {
  // Регулярное выражение для поиска аккордов
  // Ищем:
  // 1. Буквы A-G, H, а также русские буквы для обозначения аккордов (A, B, C, D, E, F, G, H, А, Б, В, Г, Д, Е)
  // 2. За которыми может следовать # или b (диез или бемоль)
  // 3. За которыми могут следовать модификаторы аккорда (m, dim, maj7, sus4, и т.д.)
  // 4. Которые выглядят как отдельные слова или в скобках
  const chordRegex = /\b([A-GHА-Е][#b]?(?:m|maj|min|aug|dim|sus|add|1?[0-9])?(?:\/[A-GH])?)\b|(\([A-GHА-Е][#b]?(?:m|maj|min|aug|dim|sus|add|1?[0-9])?(?:\/[A-GH])?\))/g;
  
  // Заменяем найденные аккорды на выделенные версии
  return text.replace(chordRegex, '<code>$&</code>');
}
