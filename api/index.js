...
bot.command('start', (ctx) => {
  ...
  ctx.reply('Привет! Я бот для поиска песен в аккорднике. Соси хуй.');
});
...
bot.on('text', async (ctx) => {
  ...
  // В групповом чате обрабатываем только сообщения, если пользователь в ожидании ввода
  if (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup') {
    // Если пользователь в режиме ожидания запроса или выбора, обрабатываем сообщение
    if (userState.state === STATES.AWAITING_SEARCH_QUERY) {
      // Выполняем поиск по запросу
      await performSearch(ctx, ctx.message.text);
      return;
    } else if (userState.state === STATES.AWAITING_SELECTION) {
      ...
      // Если это не номер песни
      setUserState(userId, STATES.DEFAULT);
      await ctx.reply('❌ Неверный номер песни. Поиск отменен. Используйте /search для нового поиска.');
      return;
    }
    
    // Во всех остальных случаях в групповом чате игнорируем сообщения без команд
    return;
  }
  
  // Далее код для личных чатов (не изменяется)
