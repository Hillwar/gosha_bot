...
const searchAnswer = 'Введите название песни или часть её текста в ответе на данное сообщение';
...
bot.command('quiz', async (ctx) => {
  ...
  // Используем cleanCommandText для очистки команды
  const query = cleanCommandText(ctx.message.text, 'quiz');
  
  const animation = await animateLoading(
    ctx, 
    "🎯 Подготавливаю конкурс... ⏳", 
    ["🎯 Подготавливаю конкурс... ⏳", "🎯 Подготавливаю конкурс... ⌛", "🎯 Загружаю форму... ⏳", "🎯 Загружаю форму... ⌛"]
  );
  ...
  try {
    ...
    // Отправляем ссылку на конкурс
    await ctx.reply('🎯 Конкурс "Первые слова":\n\nРасставьте первые строчки песен\nПервые 3 места получат достойные призы от песенной!\n\n<a href="https://forms.gle/bTVrYto47o14DHV3A">Участвовать в конкурсе</a>', {
      parse_mode: 'HTML',
      disable_web_page_preview: true
    });
    
  } catch (error) {
    console.error('Ошибка при отправке ссылки на конкурс:', error);
    try {
      if (animation.messageId) {
        await ctx.telegram.editMessageText(
          animation.chatId, 
